...
把运行前检查接入平台可用性规划：在创建 Web 任务和消费配额前检查 Chrome、目录权限、profile 占用、官方页面 origin、登录态和基本页面契约。检查失败时只跳过 Web 平台，其他可用平台继续；没有其他平台时不创建等待任务。
...
- [x] Web preflight 失败发生在任务创建和配额消费之前；其他 API 平台仍可继续运行。
