...
> 历史规则说明：本 issue 验收时采用的“仅跳过不可用 Web”行为已于 2026-07-29 退役。当前正式规则为任一所选 Web 平台实时预检失败时整体阻断，不创建部分任务或消费配额。
...
把运行前检查接入平台可用性规划：在创建 Web 任务和消费配额前检查 Chrome、目录权限、profile 占用、官方页面 origin、登录态和基本页面契约。本文原有“检查失败时只跳过 Web 平台”的行为已退役，当前正式规则为整体阻断。
...
- [x] 历史验收（已退役）：Web preflight 失败发生在任务创建和配额消费之前；当前正式规则不再继续其他 API 平台。
