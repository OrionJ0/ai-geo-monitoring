...
> 现行规则覆盖（2026-07-31）：本文关于项目/问题平台范围和 Web 实时预检整批阻断的设计已退役；当前正式入口统一使用全局启用平台，单个平台不可用时只跳过该平台。现行方案见 `../closed-2026-07-31-002-single-brand-platform-runtime/`。

> 实施状态（2026-07-30）：注册表、隔离运行时、豆包 Adapter、统一登录命令、设置页账号管理、动态状态 API、正式项目/问题集/自动监测入口、多 Store 证据、双 FIFO 和本地真实单问题验收已完成。产品决策已将新初始化的全部平台预置调整为默认停用，必须由管理员主动启用；已有数据库保留管理员保存的启停状态。目标 VM 的人工登录已验证，问题集、自动监测、双 Chrome 运行资源和正式入口全链路仍需继续验收。
...
- `doubao-web` 内置平台、能力声明、默认停用预置和配置保护。
...
- REQ-002：新初始化的 `doubao-web` 默认停用且标记为内置预置，必须由管理员主动启用；不配置 API Key，不具备分析、模型目录、连接测试和旧入口能力；已有数据库的启停值不被预置同步覆盖。
...
- 操作列提供登录或打开 Chrome、切换账号、验证登录和刷新配置信息；“刷新配置信息”只读取浏览器配置、Profile 初始化状态和上次验证结果，不检查当前登录。
...
- KTD-010：人工启用与运行验收分离。平台开关由管理员主动打开不代表登录或目标机器运行条件有效；预检失败必须显式呈现，目标虚拟机登录、资源与真实页面仍需独立验收。
...
将 DeepSeek 单例重构为平台注册表管理的独立实例，并注册默认停用、需管理员主动启用的内置 `doubao-web`。
...
- 豆包 Web 新初始化时默认停用且不需要 API Key，管理员可以主动启用。
- 一个实例关闭或熔断不改变另一个实例。
- 应用关闭会等待并回收两个实例。

**验收方式：**

平台目录可看到默认停用的豆包网页版；DeepSeek 既有测试保持通过；正式路由不再直接引用 DeepSeek 单例。
...
在目标虚拟机由管理员主动启用平台后，从真实用户入口完成登录、单问题、问题集、重试、自动监测和双 Web 并行验收。
...
- 验收失败时保留明确失败状态，由管理员按运行需要临时停用并直接修复新实现，不启用 API fallback。
- 全部通过后更新当前正式路径文档并关闭需求。
...
以真实报告、状态页面、截图、运行日志和进程证据证明正式入口走豆包 Web；只有目标 VM 全流程通过后才将需求目录转为 `closed`。

## 8. 验收标准

- AC-001：Given 全新数据库初始化，When 执行内置平台同步，Then 存在默认停用且标记为预置的 `doubao-web`，并且不需要 API Key；必须由管理员主动启用。
...
- AC-019：Given 已有数据库中的管理员曾停用 `doubao-web`，When 启动或同步预置配置，Then 已保存的停用状态保持不变。
- AC-020：Given 全新数据库和可用登录会话，When 管理员主动启用豆包网页版后用户创建项目，Then 已启用且可用的豆包网页版作为默认项目平台并可从正式入口运行。
...
- 风险：两个 headed Chrome 超出虚拟机资源。
  缓解：U1 先建立资源基线，U8 再做真实并行验收；未通过时由管理员临时停用豆包 Web 并修复运行环境，不偷偷改为共享浏览器或 API 回退。
...
- 风险：运营误把管理员启用或自动化测试通过当作目标 VM 已验收。
  缓解：设置页分开展示启用状态与登录验证状态；只有 U8 真实入口证据完整后才能关闭需求。
