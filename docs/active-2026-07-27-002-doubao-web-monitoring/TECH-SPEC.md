...
> 实施状态（2026-07-29）：注册表、隔离运行时、豆包 Adapter、统一登录命令、设置页账号管理、动态状态 API、正式项目/问题集/自动监测入口、多 Store 证据、双 FIFO 和本地真实单问题验收已完成。产品决策已将新初始化的 `doubao-web` 调整为默认启用的内置预置平台；已有数据库保留管理员保存的启停状态。目标 VM 的人工登录恢复、问题集、自动监测、双 Chrome 资源基线和正式入口验收仍未完成。
...
- KTD-010：默认启用与运行验收分离。平台开关默认打开不代表登录或目标机器运行条件有效；预检失败必须显式呈现，目标虚拟机登录、资源与真实页面仍需独立验收。
...
将 DeepSeek 单例重构为平台注册表管理的独立实例，并注册默认启用的内置 `doubao-web`。
...
- 豆包 Web 新初始化时默认启用且不需要 API Key。
- 一个实例关闭或熔断不改变另一个实例。
- 应用关闭会等待并回收两个实例。

**验收方式：**

平台目录可看到默认启用的豆包网页版；DeepSeek 既有测试保持通过；正式路由不再直接引用 DeepSeek 单例。
...
在目标虚拟机从真实用户入口完成默认启用状态下的登录、单问题、问题集、重试、自动监测和双 Web 并行验收。
...
- AC-001：Given 全新数据库初始化，When 执行内置平台同步，Then 存在默认启用且标记为预置的 `doubao-web`，并且不需要 API Key。
...
- AC-020：Given 全新数据库和可用登录会话，When 用户创建项目，Then 默认启用的豆包网页版进入可选平台并可从正式入口运行。
...
- 风险：运营误把默认启用或自动化测试通过当作目标 VM 已验收。
