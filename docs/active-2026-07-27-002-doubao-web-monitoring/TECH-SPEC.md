...
> 实施状态（2026-07-28）：注册表、隔离运行时、豆包 Adapter、统一登录命令、设置页账号管理、动态状态 API、正式项目/问题集/自动监测入口、多 Store 证据、平台默认值与双 FIFO 自动化验收已完成。登录验证已覆盖“深入研究”能力，匿名 Profile 会返回需要登录；设置页已区分只读的“刷新配置信息”与实时的“验证登录”。`doubao-web` 仍默认关闭；目标 VM 的真实引用 DOM、人工登录恢复、双 Chrome 资源基线和正式入口验收未完成。
