...
将当前只服务 DeepSeek 的 Web 单例收敛为代码内受管平台注册表和可按平台实例化的运行内核，并注册内置 `doubao-web`。2026-07-29 产品决策将其新初始化默认值调整为启用；已有管理员启停选择保持不变。
...
- [x] 平台初始化后存在内置 `doubao-web`，名称为“豆包网页版”；新初始化默认启用，已有管理员启停状态不被同步覆盖。
