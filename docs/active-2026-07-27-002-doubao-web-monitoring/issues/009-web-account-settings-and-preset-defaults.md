...
平台预置默认值同步调整：千问 Responses 请求预置强制搜索，DeepSeek API 预置模型为 `deepseek-v4-pro`，但包括两个 Web 平台在内的全部新预置默认停用，必须由管理员主动启用。默认值不能覆盖管理员后续明确保存的自定义配置或启停状态。
...
- [x] 豆包 Web 新初始化时作为内置预置平台默认停用，必须由管理员主动启用；已有管理员启停状态在预置同步后保持不变。
