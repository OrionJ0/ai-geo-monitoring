...
- 2026-08-04 脱敏核验到联系人接口可返回 `sourceChannel=organic_search` 与必应原始来路；本地九键分类器将该记录识别为 `BING_SEARCH`。原始 URL 只在按需审计详情中保留，统计快照只保存键和数量。本轮改动尚未部署。
...
4. 旧版官网适配器和缓存已部署，但生产尚未注入专用官网项目与只读账号凭据，因此模块保持 `DISABLED`。以联系人列表统计全部记录的 `v3` 快照合同仅在本地实现；正式页面不会在部署前使用它。
5. 2026-08-04 九键分类器、联系人归因字段校验、原始 `referrer` 证据展示、首页与咨询页全部记录九键统计合同仅完成本地实现和测试；尚未提交、推送、部署或从正式域名验收。
