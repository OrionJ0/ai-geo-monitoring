...
> 2026-08-04 生产说明：页面已改为服务端 `/website-traffic-overview` 与 `/website-traffic-pages` 合同并部署，包含汇总、单指标趋势、来源质量、入口/受访页面、搜索、排序和分页，不再读取旧 `/tongji-trend`。正式页已返回百度统计真实访问、UV、PV、质量和页面数据；来源行质量和来源—页面联动没有可信证据时继续显示缺失。
