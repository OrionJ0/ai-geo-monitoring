...
- 搜索推广与百度统计都使用同一连接的版本化 OAuth Access Context；只保存一套加密 Access/Refresh Token。百度统计另保存非秘密商业账号用户名，用户名变更必须使用当前 OAuth Token 实时通过站点目录校验。旧独立统计 Token、写路由和 fallback 已退役。
...
1. 历史/已退役：2026-08-03 曾用临时独立百度统计 Data API Token 做只读能力验证，确认商业 API 返回三个活动站点，其中 `siteId=23412673` 的域名为 `gato.com.cn`；由此否定并退役了“唯一活动站点”假设。当前生产已改为统一 OAuth 与明确项目绑定，不再保存或使用该独立 Token。
2. 历史/已退役：同日对 `siteId=23412673` 只读调用 `source=through`、`source=search,0`、`source=link` 均返回成功且当时为诚实零数据。当前持续观察使用统一 OAuth Access Context；`--` 仍保持缺失，不转成零。
