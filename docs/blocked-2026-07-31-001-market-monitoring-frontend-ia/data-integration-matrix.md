...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 市场总览 `/geo/market-overview`：ROAS、CPL、CPA、CPC；按渠道的广告投入、展现、官网访问、官网咨询、线索入池、成交订单/金额、整体转化率；全部/单渠道每日访问趋势与渠道对比 | `useMarketOverview`、`useWebsiteTrafficOverview`、`useWebsiteFormConsultations`、`useWebsiteFormConsultationDays` | `GET /api/marketing/projects/:projectId/dashboard`、`website-traffic-overview?includeSourceComparison=true`；`GET /api/website-data/projects/:projectId/form-consultations`、`form-consultation-days` | 百度营销四份报告 2290316/2284618/2602783/2307838；百度统计来源汇总和趋势报告；官网 `GET /api/v1/admin/contact/list` | 百度营销/统计已生产真实接入；百度推广投入/展现来自营销报告，访问来自百度统计，不用点击冒充。官网全部表单记录九键统计代码已部署并完成本地真实对账，但生产配置仍为 `DISABLED`。53KF、线索、订单未接入；销售范围明确排除 | 后端营销开关、项目白名单、百度 OAuth/统计绑定已配置；官网仍缺 `GATO_WEBSITE_FORM_PROJECT_ID` 与专用只读账号凭据；生产 fixture 关闭 | JWT + 项目所有权；百度 Token 仅数据库密文；官网凭据仅服务端；统计快照只保存日期、九键和数量 | 2026-08-05 首页本地候选已硬切到区间来源比较合同，旧固定趋势路由退役；生产状态以本需求的正式发布验收为准，官网和销售字段继续诚实缺失 |
| 广告表现 `/geo/ad-performance`：总消费、总展现、总点击、CTR、平均 CPC；每日趋势；计划→单元→关键词下钻与搜索 | `useAdPerformance`、`adPerformanceAdapter` | `GET /api/marketing/projects/:projectId/dashboard` | 百度营销四份独立报告；搜索词只关联单元并保留关键词名称证据 | 生产真实接入；开发 fixture 可显式启用但生产关闭 | `MARKETING_MONITORING_*`、百度连接/绑定与合同版本；`NEXT_PUBLIC_AD_PERFORMANCE_FIXTURE` 生产不得为 true | JWT + 项目所有权；Token 密文；响应使用精确整数/金额缩放字符串 | 正式页显示截至 2026-08-03 的真实汇总，并由同一 Dashboard revision 提供计划、单元、关键词与独立搜索词；快照按最近 30 个完整日双读、全成全败刷新 |
| 关键词分析 `/geo/keyword-analysis`：有展现/点击关键词、点击覆盖率、未获点击；消费、展现、点击、CTR、平均 CPC；象限、行动建议、单元/区间/异常筛选 | `useKeywordAnalysis`、`keywordAnalysisAdapter` | `GET /api/marketing/projects/:projectId/dashboard` 的 `keywords` | 百度营销关键词报告 2602783 | 生产真实接入；开发 fixture 仅用于隔离 UI 验收 | 同营销配置；`NEXT_PUBLIC_KEYWORD_ANALYSIS_FIXTURE` 生产不得为 true | JWT + 项目所有权；无 Token 下发 | 登录后正式页显示 863 条有展现关键词、237 条有点击关键词、点击覆盖率 27.46%，且不存在“广告关键词数据尚未开放”；新建浏览器页控制台无 error/warn |
| 网站流量 `/geo/website-traffic`：访问次数、访客数、PV、跳出率、平均访问时长、平均访问页数；当前/上期趋势；来源质量；入口/受访页面、搜索/排序/分页 | `useWebsiteTrafficOverview`、`useWebsitePageReport` | `GET /api/marketing/projects/:projectId/website-traffic-overview`、`website-traffic-pages` | 百度统计 `source/all/a`、`source/engine/a`、全站质量、入口页和受访页报告 | 生产真实接入；无 fixture 默认路径 | 百度统计凭据/站点绑定保存在独立表；营销开关和项目白名单 | JWT + 项目所有权；统计凭据数据库密文；页面 URL 需严格解析并排除跨域污染 | 正式页返回当前 30 日访问 53、UV 45、PV 112、跳出率 54.0%、平均时长 03:32 和 22 条入口页面记录；无法证明的来源级质量保持缺失 |
| 咨询数据 `/geo/consultations`：官网表单和在线客服独立摘要、趋势/来源、原始咨询记录筛选/搜索/排序/分页、按需详情抽屉 | `useConsultationRecords` + 官网逐日 hook | `GET /api/consultations/status`、`GET /api/consultations/projects/:projectId/records`、`records/:recordId`；官网逐日统计 API | 官网真实存在 `GET /api/v1/admin/contact/list`、`GET /api/v1/admin/contact/:id`；53KF 官网公开说明有开放 API 和聊天记录，但未提供当前账户可调用端点/权限/字段证据 | 官网脱敏记录与全部表单九键统计代码已部署，生产因凭据未配置为 `DISABLED`。53KF 为独立 `NOT_CONNECTED`；整体覆盖为 `PARTIAL` | 官网需独立服务端项目与专用只读账号凭据；53KF 尚无账号级只读凭据和端点配置 | Header-only JWT + 项目所有权；详情先检查审计 schema；姓名/电话/邮箱服务端脱敏；统计快照仅保留键和数量 | 2026-07-05 至 2026-08-04 本地统计 21 条，`DIRECT=3`、`BING_SEARCH=1`、`UNKNOWN=17`；正式页仍显示官网不可用与 53KF 未验证，未把自动问候/窗口打开计为咨询 |
...
4. 官网生产配置：联系人列表九键统计代码、迁移和页面已随 revision `6894789` 部署。`GATO_WEBSITE_FORM_PROJECT_ID`、专用只读用户名和密码仍未注入；启用前还需确认上游最小权限角色、轮换和撤权责任。

## 2026-08-04 本地验收摘要

- 官网真实只读：2026-07-05 至 2026-08-04，联系人列表共 21 条；本地新版区间与 31 日逐日统计均为 21，来源为 `DIRECT=3`、`BING_SEARCH=1`、`UNKNOWN=17`。验证只输出聚合数量，原始响应和凭据未写入日志、截图或本文。
- production build 已通过九键合同构建，市场总览与咨询页浏览器 fixture 共 14 项验收通过；正式 Git Bundle workflow `30900162256` 已把该代码包含在 revision `6894789` 中部署。生产模块仍为 `DISABLED`，因此市场总览和咨询页不会读取真实官网 21 条口径。
- AI/GEO/SEO 回归：项目看板、来源、SEO 检测、问题集报告分别命中各自既有内部 API 并返回 200；没有新增营销数据拼接或 fixture 回退。
- 自动化门禁：后端 994/994、营销 144/144、前端单元/合同 72/72、部署专项 26/26、Playwright 27/27；lint 和 38 路由 production build 通过。
- 本地限制：没有复制生产百度 Token；本机使用脱敏 fixture 验证严格合同、快照、鉴权与异常路径，真实四报表由服务器密文 Token 在生产完成只读刷新。官网本地独立只读凭据已验证，但不得复制为生产默认值。
- 生产验收：workflow `30876793311` 已把 `f265bd3` 快进部署，迁移审计与两个 systemd 服务通过；公网 ready 与 frontend revision 正常，登录后市场、百度统计与 AI/GEO 页面返回真实数据。官网、53KF 和销售字段分别保持 `DISABLED`、`NOT_CONNECTED` 与 `UNAVAILABLE`。
