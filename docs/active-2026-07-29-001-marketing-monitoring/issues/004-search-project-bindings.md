...
让管理员从已授权连接中选择整个百度搜索推广账户和明确的百度统计站点绑定项目，并支持暂停、恢复、改绑和解除绑定。

## Scope

- 迁移 `baidu_project_bindings`。
- 实现账户与百度统计站点目录适配、绑定列表、新增、暂停、恢复和删除。
...
- [ ] 保存和恢复前服务端重新读取或验证账户归属与统计站点状态，不信任前端名称或域名。
- [ ] 契约未证明全局 ID 前数据库按 `(project_id, connection_id, external_account_id)` 防重，服务层禁止同一 canonical account 同时属于多个活动项目。
- [ ] 绑定只接受 connectionId、externalAccountId 和 tongjiSiteId，不接受前端提供的域名、campaign ID、channel 或 scope type。
- [ ] 运行时必须读取明确绑定的活动站点；不得按“唯一活动站点”自动猜测。
