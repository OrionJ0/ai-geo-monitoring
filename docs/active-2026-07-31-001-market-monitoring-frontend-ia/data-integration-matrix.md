...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 市场总览 `/geo/market-overview`：ROAS、CPL、CPA、CPC；按渠道的广告投入、展现、官网访问、官网表单咨询、线索入池、成交订单/金额、整体转化率；每日趋势、渠道/指标切换 | `useMarketOverview`、`useWebsiteTrafficOverview`、`useWebsiteFormConsultations`、`useWebsiteFormConsultationDays` | `GET /api/marketing/projects/:projectId/dashboard`、`tongji-trend`、`tongji-source-trends`；`GET /api/website-data/projects/:projectId/form-consultations`、`form-consultation-days` | 百度营销四份报告 2290316/2284618/2602783/2307838；百度统计来源报告；官网 `GET /api/v1/admin/stats/dashboard` | 百度营销/统计已生产真实接入；百度推广投入/展现来自营销报告，访问来自百度统计，不用点击冒充。官网代码已部署、本地真实对账通过，但生产配置 `DISABLED`。53KF、线索、订单未接入；销售范围明确排除 | 后端营销开关、项目白名单、百度 OAuth/统计绑定已配置；官网仍缺 `GATO_WEBSITE_FORM_PROJECT_ID` 与专用只读账号凭据；生产 fixture 关闭 | JWT + 项目所有权；百度 Token 仅数据库密文；官网凭据仅服务端；聚合响应禁止联系人/会话明细 | 正式页显示百度推广消费 ¥13,057.37、展现 51,976、点击 13,692、CPC ¥0.95；官网模块显示不可用，销售字段继续 `UNAVAILABLE`，不计算 ROAS/CPL/CPA/转化率 |
...
| 咨询数据 `/geo/consultations`：官网表单和在线客服独立摘要、趋势/来源、原始咨询记录筛选/搜索/排序/分页、按需详情抽屉 | `useConsultationRecords` + 官网逐日 hook | `GET /api/consultations/status`、`GET /api/consultations/projects/:projectId/records`、`records/:recordId`；官网逐日聚合 API | 官网真实存在 `GET /api/v1/admin/contact/list`、`GET /api/v1/admin/contact/:id`；53KF 官网公开说明有开放 API 和聊天记录，但未提供当前账户可调用端点/权限/字段证据 | 官网代码已部署、本地真实接入；生产因凭据未配置为 `DISABLED`。53KF 为独立 `NOT_CONNECTED`；整体覆盖为 `PARTIAL` | 官网需独立服务端项目与专用只读账号凭据；53KF 尚无账号级只读凭据和端点配置 | Header-only JWT + 项目所有权；详情先检查审计 schema；姓名/电话/邮箱服务端脱敏；禁止 IP、原始 Token、完整上游响应进入日志/报告 | 本地独立凭据对 2026-08-01 至 2026-08-04 返回 3 条记录、30 日列表 20 条；正式页明确显示官网不可用与 53KF 未验证，未把自动问候/窗口打开计为咨询 |
...
4. 官网生产配置：代码、迁移和页面均已部署，但 `GATO_WEBSITE_FORM_PROJECT_ID`、专用只读用户名和密码尚未注入；启用前还需确认上游最小权限角色、轮换和撤权责任。Git Bundle 发布链路本轮已成功，不再是阻塞项。

## 2026-08-04 本地验收摘要

- 官网真实只读：2026-08-01 至 2026-08-04，聚合会话数 3、记录总数 3、返回 3；列表字段全部脱敏，详情合同不返回原始联系人和 IP。原始响应和凭据未写入日志、截图或本文。
- production build 页面：市场总览命中 `/api/website-data/projects/6/form-consultations` 并展示合计 3 个官网可归因会话（直接 2、自然搜索 1）；咨询数据页所选 30 日命中逐日和原始咨询记录 API，列表返回 20 条脱敏表单记录，并明确显示官网与 53KF 两个独立状态；订单页没有订单 API 请求并显示不可用。20 条原始记录不等于 3 个可归因会话。
