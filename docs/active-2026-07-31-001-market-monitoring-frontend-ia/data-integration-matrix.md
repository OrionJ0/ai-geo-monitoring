...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 市场总览 `/geo/market-overview`：ROAS、CPL、CPA、CPC；按渠道的广告投入、展现、官网访问、官网表单咨询、线索入池、成交订单/金额、整体转化率；每日趋势、渠道/指标切换 | `useMarketOverview`、`useWebsiteTrafficOverview`、`useWebsiteFormConsultations`、`useWebsiteFormConsultationDays` | `GET /api/marketing/projects/:projectId/dashboard`、`tongji-trend`、`tongji-source-trends`；`GET /api/website-data/projects/:projectId/form-consultations`、`form-consultation-days` | 百度营销四份报告 2290316/2284618/2602783/2307838；百度统计来源报告；官网 `GET /api/v1/admin/contact/list` | 百度营销/统计已生产真实接入；百度推广投入/展现来自营销报告，访问来自百度统计，不用点击冒充。官网全部表单记录九键统计已完成本地真实对账，但新版未部署且生产配置 `DISABLED`。53KF、线索、订单未接入；销售范围明确排除 | 后端营销开关、项目白名单、百度 OAuth/统计绑定已配置；官网仍缺 `GATO_WEBSITE_FORM_PROJECT_ID` 与专用只读账号凭据；生产 fixture 关闭 | JWT + 项目所有权；百度 Token 仅数据库密文；官网凭据仅服务端；统计快照只保存日期、九键和数量 | 2026-07-05 至 2026-08-04 本地只读返回 21 条，`DIRECT=3`、`BING_SEARCH=1`、`UNKNOWN=17`，逐日合计一致；正式页仍显示官网模块不可用，销售字段继续 `UNAVAILABLE` |
