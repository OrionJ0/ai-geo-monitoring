...
4. 官网生产配置：旧版代码、迁移和页面已部署；联系人列表九键统计新版尚未部署。`GATO_WEBSITE_FORM_PROJECT_ID`、专用只读用户名和密码仍未注入；启用前还需确认上游最小权限角色、轮换和撤权责任。
...
- production build 已通过本地新版合同构建；暂存区独立副本的市场总览与咨询页浏览器 fixture 共 14 项验收通过。正式环境尚未部署联系人列表九键统计，因此市场总览和咨询页目前不会使用新版 21 条口径。
