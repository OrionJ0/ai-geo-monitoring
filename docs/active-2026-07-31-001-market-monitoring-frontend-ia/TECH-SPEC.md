...
- `nextjs-frontend/src/app/geo/page.tsx` 当前固定重定向到 `/geo/projects`。
- `nextjs-frontend/src/app/geo/layout.tsx` 内联维护菜单、面包屑、登录态和响应式侧栏。
...
- KTD-003：导航配置从 `geo/layout.tsx` 提取为纯配置构建器，由统一路由合同生成完整菜单、面包屑及选中态；来源能力只控制页面数据状态。理由：无数据时仍需让内部用户理解完整业务结构，同时避免名称和选中态在多处漂移。
...
**目标：** 新分组、改名、面包屑和选中态从同一配置产生；`/geo` 固定进入市场总览，来源状态只控制数据读取。
...
- 纯构建器生成菜单、面包屑和路径选中态。
- 所有阶段按市场总览、投放与流量、转化结果、AI 品牌监测、网站诊断、监测任务、快捷导航和设置的顺序显示。
- 分组使用 Ant Design `SubMenu`，当前路由所属分组自动展开；各分组独立维护展开状态，不强制关闭其他分组。
- 应用壳固定为一个视口高度：Header 固定，Sider 占满 Header 下方空间，根页面禁止滚动，仅 Content 承担主页面纵向滚动。
- 一级直接菜单与一级分组标题共用相同中性灰、字号和字重；末级当前项使用统一选中态。
- `/geo` 固定进入市场总览；未开放来源在页面内显示中性占位状态。
- 工作台设置分组仅保留系统通知和个人中心，不跳转管理员设置中心。
- 快捷导航按投放与访问、接待与线索、官网与采购三组静态维护九个来源系统入口；分组标题使用完整内容行，卡片网格另起一行。图标优先读取来源站点 favicon 或官方品牌资源，加载失败时显示本地文字回退。外部地址使用新标签页和 `rel="noreferrer"`，未知 Agent 地址显示不可操作状态。
- 改名只改变用户文案，保留现有 GEO 路由。

**测试场景：**

- 完整菜单和固定默认入口。
- 多分组同时展开、当前路由自动展开和移动端重新打开时恢复当前分组。
- 根页面无纵向滚动、侧栏位置固定、主内容区独立滚动。
- 直接子路由的选中态和面包屑。
- 设置分组不存在管理员后台跳转。
- 快捷导航的九个系统、官方地址、新标签页安全属性和未知地址状态。
- 窄屏折叠、键盘和焦点。
...
- AC-013：Given 管理员进入后台，When 使用桌面或移动端侧边栏，Then 可返回数据工作台，账号与权限、系统管理可独立展开且当前页面所属分组自动打开。
- AC-014：Given 页面内容超过视口，When 用户滚动页面，Then Header 和 Sider 位置不变，HTML 与 body 不滚动，只有 Content 的滚动位置变化。
- AC-015：Given 多个一级分组，When 用户依次展开，Then 已展开分组保持打开，一级直接项与分组标题的基础字体样式一致。
- AC-016：Given AI 搜索表现数据 section，When 页面渲染，Then section 不存在装饰性 `::before` 色条。
- AC-017：Given 用户进入快捷导航，When 页面渲染，Then 九个指定系统完整出现，分类标题位于卡片网格上方且每个网站有图标，已知入口安全打开新标签页，未知 Agent 地址没有虚构链接。
- AC-018：Given 用户查看一级导航，When 比较直接入口和分组标题，Then 文字使用一致的中性灰；设置分组不包含管理设置。
...
- 管理员后台导航名称、选中态、多组展开、返回工作台和移动端覆盖行为。
