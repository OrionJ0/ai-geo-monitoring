...
新版目标按来源展示“广告投入 / 展现 → 访问（点击） → 官网表单咨询 / 在线客服咨询 → 线索入池 → 成交订单”，官网表单与 53KF 在线客服是独立事实。成交订单同时提供订单数和订单金额。CPA、成交率和整体转化率使用成交订单数，ROAS 使用成交订单金额。只有可信来源键或确认映射成立时，记录才进入同一来源链路。

当前仓库已完成新版页面视觉、交互、状态、响应式与无障碍实现，包含百度推广行、CPC、百度统计来源行和趋势，以及独立官网表单区间/逐日聚合接口与“官网表单咨询”列。官网模块使用 `/api/website-data`、独立配置/服务/缓存仓储/迁移账本和前端 hook；百度继续使用 `/api/marketing`。官网聚合只返回来源级可归因成功提交会话；另由 `/api/consultations` 的官网 adapter 按需读取真实列表/详情并在服务端脱敏，仍不混入 53KF。`BAIDU_PAID` 与 `DIRECT` 精确对齐，未细分来源单独成行。2026-08-01 至 2026-08-04 已使用独立只读凭据完成 3 条记录的聚合、列表、详情与页面对账，本地页面所选 30 日原始表单列表另返回 20 条；两者保持不同指标口径。上游仍没有可验证的只读角色合同。53KF、客服转线索、订单结果接口合同仍缺失，ROAS/CPL/CPA 仍为诚实缺失状态。第二阶段业务变更尚未推送、部署或生产验收。

同次盘点确认广告表现、关键词分析、网站流量、原始咨询和订单结果均已有本地设计实现。原始咨询页消费严格的 `consultation_records_v1` 浏览器合同；配置完整时官网 adapter 为 `AVAILABLE/FULL`，失败时与 53KF 独立降级，53KF adapter 固定为 `NOT_CONNECTED`。订单结果没有网络客户端，生产数据源固定为 `UNAVAILABLE`，完整数据仅能在 `NODE_ENV !== 'production'` 时显式启用 fixture。逐页状态统一见 `../README.md#当前前端页面实施状态`，本文件不再把这两个页面描述为“尚未实现的占位页”。
...
- `/geo` 默认入口解析。
- 新增市场总览、广告表现、关键词分析、网站流量、原始咨询和订单结果页面；关键词分析只指百度投放关键词，不把用户搜索词或推广计划聚合冒充关键词事实。
- 原始咨询与订单结果在真实来源缺失时仍提供完整、诚实的页面状态；非生产 fixture 只用于视觉与交互验收，不进入生产默认路径。
...
- 原始咨询读取 `consultation_records_v1` 的来源状态、列表和详情合同；配置完整时官网为 `AVAILABLE/FULL`，53KF 为 `NOT_CONNECTED`，因此正式列表只包含脱敏官网记录并显示部分覆盖原因。
...
- 关键词分析页读取同一 Dashboard revision 的 `keywords` 范围聚合，不再停留在开发 fixture；它只消费投放关键词 ID/名称和父级 ID，不读取或混入 `searchTerms`。
...
4. 官网聚合统计接口、最多 31 日逐日聚合、脱敏记录级列表/详情已完成本地实现与真实区间及页面对账；官网上游仍没有全部表单记录数和可验证的只读角色合同，生产凭据配置、部署和正式入口验收仍未完成。
