...
1. 2026-08-03 使用临时本地 Token 只读验证：百度统计商业 API 返回三个活动站点，其中 `siteId=23412673` 的域名为 `gato.com.cn`；因此“唯一活动站点”假设已被否定并退役。正式数据库中的 OAuth Token、项目绑定和生产入口仍需完成本次部署后验收。
2. 2026-08-03 对 `siteId=23412673` 只读调用 `source=through`、`source=search,0`、`source=link` 均返回成功且当前为诚实零数据；网站安装统计代码后的数据到达、限流与生产 OAuth Token 下的持续表现仍待观察。
