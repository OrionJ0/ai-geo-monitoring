...
- Git Bundle workflow `30876793311` 已成功，发布链路不再阻塞；生产已运行 `f265bd3`。
- 官网代码与迁移已部署，但生产仍缺专用官网项目与只读账号凭据，模块为 `DISABLED`。
- 53KF 仍缺当前账户可执行的只读 API 合同和凭据；销售系统按范围约束保持 `UNAVAILABLE`。

## 本地最终证据（2026-08-04）

- 测试：后端 994/994、营销 144/144、前端单元/合同 72/72、部署专项 26/26、Playwright 27/27；lint 和 38 路由 production build 通过。
