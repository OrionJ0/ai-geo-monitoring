...
- 正式分析契约为 `ai_structured_v4`；本修复前提示词修订为 `semantic_evidence_few_shot_v6`，实现后升级为 `semantic_evidence_few_shot_v7`。
...
- 本目录已完成全部生产门禁并进入 `closed`；正式入口、默认路径和运行证据见下节。
