...
- [ ] 2026-07-27：完成目标 VM 的豆包 Web 正式切换
  ...
  - 修复进展：提交 `169de9c494e7` 已在目标 VM 完整发布；设置页已包含豆包 Web、登录/切换账号、验证登录和重新加载，Qwen/DeepSeek 默认值也已生效。当前 DeepSeek Web 与豆包 Web 的专用 Profile 均为“需要登录”，因此只验证了失败链路，尚未完成真实采集成功验收。
  - 下一步：在目标 VM 的两个专用 Chrome 中完成登录，点击“验证登录”确认状态为可用，再从隔离项目运行单问题与问题集，核对回答、来源、运行报告、管理历史和数据库记录，证明 Web 通道成功且未走 API。
...
- [x] 2026-07-28：豆包新对话地址缺少尾斜杠时被误判为页面结构变化
  ...
  - 验证结果：通过目标 VM 受管 Chrome 的只读 CDP 页面证据确认 `pathname=/chat`、助手回答数为 0；新增当前路径回归测试先失败后通过，待重新部署后从真实单问题入口继续验证深入研究和回答采集。
...
- [x] 2026-07-28：修复有效的豆包 Web 配置在正式运行入口被误判为无效
  ...
  - 验证结果：新增非枚举 Sequelize getter 配置回归测试先失败后通过；定向后端测试通过，待目标 VM 重新部署后从同一真实单问题入口复验。
...
- [x] 2026-07-28：运行报告明确展示被跳过的平台
  ...
  - 验证结果：新增跳过平台格式化与报告页面回归测试先失败后通过；待目标 VM 重新部署后验证真实报告中的 DeepSeek Web 登录提示。
...
- [x] 2026-07-28：标明项目引用的已停用平台并给出可操作提示
  ...
  - 验证结果：新增平台状态展示、摘要和目录加载边界测试先失败后通过；相关定向测试及前端完整 238 项测试通过。
...
- [x] 2026-07-28：补全 Web 会话最近验证与账号隐私说明
  ...
  - 验证结果：新增验证时间与账号隐私展示测试先失败后通过；前端完整 238 项测试、Lint 和生产构建通过。
