...
> 2026-07-30 抓取安全更新：正式入口已增加响应可信度分类、WAF/429 任务内止损、有界预检和 resolved URL 去重；完整当前合同与验收见 `../closed-2026-07-30-001-seo-audit-response-safety/`。2026-07-31 根据真实站点 1 页任务仍耗时 111.2 秒的基线，将无风控任务调整为页面并发 4、同域 250ms 间隔，并让 Sitemap 递归和站内链接探活受实际页面范围约束；站外链接退出全站主请求链路。随后真实自有站诊断确认一条 EdgeOne 信号正则会在正常的大型单行 HTML 上重复扫描正文，已改为线性 token 检查并加入 270KB 页面性能回归。
>
> 2026-07-31 礼貌抓取补充：单页和全站正式入口都会先验证 `GoodieAI-SEO-Audit` 对提交 URL 的 robots 权限；每个入口重定向目标在请求页面前也会验证目标 origin 的 robots。明确 `Disallow` 的页面不会收到页面请求。单实例内的审计任务共享同域启动间隔和 WAF/429 冷却状态；429 按 `Retry-After` 恢复，WAF 使用短期冷却。该协调仍是进程内实现，不宣称跨多实例。
...
- `crawl`：全站页数上限 200、页面并发 4、同域请求启动间隔至少 250ms、Sitemap 上限 20、递归深度 3、站内链接网络探活至少 10 个、每个成功页面 2 个且全任务最多 50 个、浏览器渲染抽样上限 3。
...
- HTTP 重定向链与循环。
- 优先复用已检测页面状态，并只对未覆盖站内链接抽查；站内链接网络探活至少 10 个、每个成功页面 2 个且全任务最多 50 个，未完整覆盖时明确返回证据不足。站外链接保留在逐页数据中，但不由主任务发出网络请求。
...
本实现只对最多 3 页执行浏览器渲染抽样；内链检查复用已检测页面状态，额外网络探活最多 50 个未覆盖站内目标，未完整覆盖时不会宣称全部内链健康。站外链接不发出网络请求，也不提供外链可用性结论。它不调用搜索平台账号 API，也不提供真实 Core Web Vitals、关键词排名、收录量、域名权重或外链数据库。后续如加入 Lighthouse/CrUX，应作为独立重任务证据，不得把当前服务器响应时间改名为 Core Web Vitals。
