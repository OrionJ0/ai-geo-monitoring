...
> 2026-07-30 抓取安全更新：正式入口已增加响应可信度分类、WAF/429 任务内止损、有界预检和 resolved URL 去重；完整当前合同与验收见 `../closed-2026-07-30-001-seo-audit-response-safety/`。2026-07-31 根据真实站点 1 页任务仍耗时 111.2 秒的基线，将无风控任务调整为页面并发 4、同域 250ms 间隔，并让 Sitemap 递归和链接探活受实际页面范围约束。
...
- `crawl`：全站页数上限 200、页面并发 4、同域请求启动间隔至少 250ms、Sitemap 上限 20、递归深度 3、链接网络探活至少 10 个、每个成功页面 2 个且全任务最多 50 个、浏览器渲染抽样上限 3。
...
- HTTP 重定向链与循环。
- 优先复用已检测页面状态，并对未覆盖站内链接优先抽查；链接网络探活至少 10 个、每个成功页面 2 个且全任务最多 50 个，未完整覆盖时明确返回证据不足。
...
本实现只对最多 3 页执行浏览器渲染抽样；链接检查复用已检测页面状态，额外网络探活最多 50 个未覆盖目标，未完整覆盖时不会宣称全部链接健康。它不调用搜索平台账号 API，也不提供真实 Core Web Vitals、关键词排名、收录量、域名权重或外链数据库。后续如加入 Lighthouse/CrUX，应作为独立重任务证据，不得把当前服务器响应时间改名为 Core Web Vitals。
