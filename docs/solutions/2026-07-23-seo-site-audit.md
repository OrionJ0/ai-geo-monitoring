...
全站模式在 v4 技术健康分之外增加 `sitewide-audit-v2` 专项层。专项结果不静默改变既有评分，单独返回检查状态、受影响 URL、检测事实与建议：
...
- 后端全量 555 项（含真实 Chrome 集成）、前端工具测试 160 项、TypeScript 检查和本次相关文件 ESLint 全部通过。
...
- `sitewide-audit-v2` 在抓取截断、Sitemap 清单未读完、链接抽查达到上限或 hreflang 目标未进入审计范围时返回“证据不足”，不再把未覆盖部分当作通过。
