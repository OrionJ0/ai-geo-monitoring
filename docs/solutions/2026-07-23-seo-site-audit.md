...
> 2026-07-30 抓取安全更新：正式入口已增加响应可信度分类、WAF/429 任务内止损、同域 500ms 间隔、有界预检和 resolved URL 去重；完整当前合同与验收见 `../closed-2026-07-30-001-seo-audit-response-safety/`。
...
- `crawl`：全站页数上限 200、页面并发 2、同域请求启动间隔至少 500ms、Sitemap 上限 20、递归深度 3、唯一链接状态抽查上限 500、浏览器渲染抽样上限 3。
