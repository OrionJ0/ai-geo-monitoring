...
- 历史 v3 入口证据：登录后从侧边栏进入 SEO 检测页，对 `https://gato.com.cn/` 检测成功，得到报告 #4、HTTP 200、61 分、9 个问题和 6 个分类；其 `sitemap.xml` 虽返回 200，但实际是没有任何 `loc` 的空 `urlset`，当时显示“Sitemap 格式无效”和“未发现有效 loc”。当前 v4 文案已改为“Sitemap 中没有有效页面地址”，该历史报告不会被追溯改写。
