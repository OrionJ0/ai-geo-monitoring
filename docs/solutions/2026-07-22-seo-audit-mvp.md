...
> 状态：历史/已退役。本文保留 2026-07-22 单页 MVP 的决策与验证证据，不再代表当前正式流程。当前默认入口已升级为全站异步检测，见 [全站技术 SEO 审计：正式实现与验证](2026-07-23-seo-site-audit.md)。

## 目标

在现有 GEO 工作台中增加一个能直接指导修改的 SEO 检测页。用户输入公开页面 URL 后，系统优先告诉用户“先修什么”，再展示基础分和分类明细。

当时版本定位为单页关键项检测，不宣称替代全站技术审计、真实用户性能监测、关键词排名或外链数据库。
...
2. 结果按用户可理解的类型分组，使用“检查对象 → 具体问题 → 检测事实 → 小字建议”的信息层级，避免把建议或理想状态当成问题标题。
...
| 分类 | 检查项 | 失败优先级 |
| --- | --- | --- |
| 收录与抓取 | HTTP 状态、noindex、HTTPS、robots.txt、Sitemap 内容、搜索平台 HTML 验证标签 | 严重 / 高 / 中 / 低 |
...
规则不采用“资源存在即通过”：`robots.txt` 必须有非空且可识别的 `User-agent` 规则；Sitemap 必须是非空 XML，并在 `urlset` 或 `sitemapindex` 中包含有效的绝对 `loc`，`robots.txt` 声明的自定义地址也要实际抓取；Title、Meta Description、Canonical、H1、Open Graph 和图片 Alt 会去除空白后判断；JSON-LD 必须是非空且能解析为 JSON 对象。页面源码中的 Google、Bing、百度、360、搜狗和 Yandex 验证标签仅作为“发现相关 HTML 信号”，不等同于搜索平台后台已接入；DNS 和验证文件方式也不在单页 HTML 的可观察范围内。
...
SeoSiteClient ── 安全解析、固定 DNS、手动校验重定向、限时/限量抓取
      ▼
SeoAuditService ── Cheerio 解析、21 项规则、内容有效性、权重评分、分类与优先级
      ▼
SeoAuditHistoryService ── 按用户将完整报告保存到 SQLite
      │ GET /api/seo-audits、GET /api/seo-audits/:id
      ▼
页面结果 ── 具体问题与事实 → 分数摘要 → 分类检查 → 搜索/分享预览
      └── 历史报告抽屉 ── 分页摘要 → 完整报告回放
```
...
- 后端全量测试：377 项通过，覆盖内容有效性、搜索平台标签、分析规则、路由契约、用户隔离、分页历史、私网 IP、DNS 私网解析和私网重定向拦截。
- 前端 SEO 专项 Node 测试：4 项通过，专项 ESLint 通过；前端全量测试 106 项中 105 项通过，另 1 项是问题库页面已改用“问题”但旧测试仍断言旧术语的无关在途失败。
- 前端生产构建：`/geo/seo-audit` 已被 Next.js 静态生成。
- 真实入口：登录后从侧边栏进入 SEO 检测页，对 `https://gato.com.cn/` 检测成功，得到报告 #4、HTTP 200、61 分、9 个问题和 6 个分类；其 `sitemap.xml` 虽返回 200，但实际是没有任何 `loc` 的空 `urlset`，因此正确显示“Sitemap 格式无效”和事实“未发现有效 loc”。随后从“历史报告”抽屉重新打开报告 #1，旧报告在不改写存储 JSON 的前提下兼容显示为“页面标题长度需要优化”“Meta 描述缺失”“缺少 H1”等问题语义。
...
其中“异步全站爬取”已在 2026-07-23 完成；以下列表仅保留历史计划原貌。
