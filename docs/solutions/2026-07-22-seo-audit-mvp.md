...
安全抓取层会拒绝 URL 凭据、本机与私网 IP、解析到私网的域名，并在每次重定向后重新解析和校验。连接使用已校验的 DNS 地址，避免校验后再次解析造成 DNS rebinding；同时禁用代理继承，限制 5 次重定向、10 秒超时和 2 MB HTML 响应体。
...
- 后端全量测试：368 项通过，覆盖分析规则、路由契约、私网 IP、DNS 私网解析和私网重定向拦截。
- 前端生产构建：`/geo/seo-audit` 已被 Next.js 静态生成。
- 真实入口：登录后从侧边栏进入 SEO 检测页，对 `https://gato.com.cn/` 检测成功，得到 HTTP 200、65 分、7 个问题和 6 个分类。
...
3. 保存审计快照，提供历史趋势、问题负责人、已忽略规则和复测闭环。
