...
- 当前缺口：官网表单区间、最多 31 日逐日聚合和脱敏记录级列表/详情代码已部署，并使用独立只读凭据完成本地真实调用和页面对账；生产仍缺专用官网项目与只读账号凭据，因此模块为 `DISABLED`。2026-08-04 本地工作区已把首页和咨询页统一为九键来源合同，并让咨询记录按原始 `referrer` 分类，但本轮修改尚未提交、部署或从正式入口验收。53KF 缺少当前账户可执行的只读接口合同和凭据；线索池与销售系统也未形成已验证的数据链路。
...
| 咨询数据（原始咨询记录） | 表单/在线客服独立口径、趋势/分布、记录表和详情抽屉已部署；九键来源分类与原始 `referrer` 证据展示已完成本地修改 | 官网既有代码已部署、本地真实只读验证通过，但生产配置为 `DISABLED`；九键修改尚未部署；53KF 仍为 `NOT_CONNECTED`，页面诚实返回部分覆盖，不渲染 QA fixture |
