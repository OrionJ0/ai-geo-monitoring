...
- 当前缺口：官网表单区间、最多 31 日逐日聚合和脱敏记录级列表/详情已在本地工作区通过独立 `/api/website-data` 与 `/api/consultations` 接入，并使用独立只读凭据完成真实调用和页面对账；第二阶段业务变更尚未推送、部署、配置生产凭据或完成正式入口验收。53KF 缺少当前账户可执行的只读接口合同和凭据；线索池与销售系统也未形成已验证的数据链路。
...
下表描述 2026-08-04 本地工作区，目的是把“页面已经按设计实现”和“真实数据已经接通”分开。第二阶段业务变更尚未推送、部署或从正式域名验收，不能据此推断生产已经显示这些页面或数据。

| 页面 / 入口 | 本地前端状态 | 当前数据边界 |
| --- | --- | --- |
| 工作台外壳与导航 | 已完成固定顶部栏、静态分组侧栏、移动端覆盖侧栏、面包屑规则和 `/geo` → 市场总览默认入口 | 页面可见性与来源数据可用性解耦；旧 `/geo/marketing` 已在本地代码单向重定向到市场总览 |
| 市场总览 | 新版投放效率、来源全链路、每日趋势、响应式与无障碍已完成 | 可展示百度广告、CPC、百度统计访问和官网可归因表单咨询；53KF、线索入池、订单数/金额及依赖指标仍缺失 |
| 广告表现 | 汇总、趋势、严格层级树、详情和筛选已完成 | 读取同一 Dashboard revision 的计划、单元、关键词；搜索词保持独立事实。完整视觉 fixture 仅限非生产验收 |
| 关键词分析 | 摘要、筛选、散点/密度、详情、行动分布和明细表已完成 | 读取 Dashboard 的真实投放关键词；生产缺少显式行动标签时显示未分类或 `—`，不自动生成建议 |
| 网站流量 | 汇总、趋势、来源质量、入口/受访页面、筛选和分页已完成 | 读取服务端百度统计合同；来源行质量和来源—页面关联没有可信证据时保持缺失 |
| 原始咨询 | 表单/在线客服独立口径、趋势/分布、记录表和详情抽屉已完成 | 官网聚合及脱敏记录级 API 已完成本地真实只读验证；53KF 仍为 `NOT_CONNECTED`，因此合同和页面明确返回部分覆盖，不渲染 QA fixture |
...
- `active-2026-07-29-001-marketing-monitoring/`：只读市场数据监控定位下的百度第一期；四份搜索推广层级报告及百度统计来源、质量和页面报告已完成严格合同与真实 Token 只读响应验证，`PILOT_DATA_READY` 仍只向项目白名单开放；页面入口已展示，真实数据仍等待金额/时区、Refresh Token、部署与完整生产验收
- `active-2026-07-31-001-market-monitoring-frontend-ia/`：工作台信息架构与 2026-08-03 按来源完整链路的新版首页视觉/交互已完成，设计稿见[`assets/market-overview-home-final-2026-08-03.png`](active-2026-07-31-001-market-monitoring-frontend-ia/assets/market-overview-home-final-2026-08-03.png)。官网表单区间/逐日聚合已完成本地接入、真实区间对账并与百度 API 分离；53KF、线索/订单数据、依赖这些字段的 KPI 与生产入口验收仍待完成
