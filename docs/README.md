...
- 响应与风控：页面、robots、Sitemap 和链接探活按预期类型分类，只有正常响应进入对应分析；WAF/429 不计为站点 SEO 扣分，并停止本任务对该 origin 的后续请求。默认页面并发为 2，同域请求启动间隔至少 500ms；全站递归前依次预检入口、robots 和默认 Sitemap
...
- 当前边界：浏览器渲染只抽样最多 3 页，链接状态最多抽查 500 个唯一目标；尚未执行 GoodieAI 自身 robots 授权、跨实例共享限速/熔断或总请求硬上限，也不提供真实 Core Web Vitals、关键词排名、搜索引擎收录量或外链数据库
