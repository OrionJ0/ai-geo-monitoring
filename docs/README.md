...
| 页面 / 入口 | 本地前端状态 | 当前数据边界 |
| --- | --- | --- |
| 工作台外壳与导航 | 固定顶部栏、静态分组侧栏、移动端覆盖侧栏、面包屑规则和 `/geo` → 市场总览默认入口已部署 | 页面可见性与来源数据可用性解耦；旧 `/geo/marketing` 已单向重定向到市场总览 |
| 市场总览 | 新版投放效率、来源全链路、每日趋势、响应式与无障碍已部署 | 生产展示百度广告、CPC 和百度统计真实数据；官网模块因生产凭据未配置而显示不可用；53KF、线索入池、订单数/金额及依赖指标仍缺失 |
| 广告表现 | 汇总、趋势、严格层级树、详情和筛选已部署 | 四份百度报告已生产真实刷新；搜索词保持独立事实；正式页显示截至 2026-08-03 的消费、展现和点击 |
| 关键词分析 | 摘要、筛选、散点/密度、详情、行动分布和明细表已部署 | 正式页显示 863 条有展现关键词、237 条有点击关键词，且不再显示“尚未开放” |
| 网站流量 | 汇总、趋势、来源质量、入口/受访页面、筛选和分页已部署 | 正式页返回百度统计访问、UV、PV、质量和页面数据；来源行质量和来源—页面关联没有可信证据时保持缺失 |
| 原始咨询 | 表单/在线客服独立口径、趋势/分布、记录表和详情抽屉已部署 | 官网代码已部署、本地真实只读验证通过，但生产配置为 `DISABLED`；53KF 仍为 `NOT_CONNECTED`，页面诚实返回部分覆盖，不渲染 QA fixture |
