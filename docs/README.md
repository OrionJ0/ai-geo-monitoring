...
- `solutions/2026-07-22-seo-audit-mvp.md`：SEO 检测 MVP 的竞品调研、规则范围、架构与验证记录
...
- 当前能力：对单个公开 HTML 页面执行 21 项关键基础检查，返回加权分数，以及按“检查对象 → 具体问题 → 检测事实 → 建议”组织的严重程度修复清单、六类明细和搜索/分享预览；每次成功检测按当前账户保存，并支持分页查看、回放完整报告
...
- 搜索平台：识别 Google、Bing、百度、360、搜狗和 Yandex 的非空 HTML 验证标签；该结果只表示页面源码信号，不能证明平台后台当前已验证，也无法发现 DNS 或验证文件方式
- 当前边界：不执行全站爬取，不提供关键词排名或外链数据库，也不把服务器响应耗时等同于真实 Core Web Vitals
- 详细设计与竞品取舍见 `solutions/2026-07-22-seo-audit-mvp.md`
