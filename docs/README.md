...
- 当前缺口：官网表单区间、最多 31 日逐日聚合和脱敏记录级列表/详情代码已部署，并使用独立只读凭据完成本地真实调用和页面对账；生产仍缺专用官网项目与只读账号凭据，因此模块为 `DISABLED`。2026-08-04 首页和咨询页的九键来源合同及按原始 `referrer` 分类已合并并推送到 `main`，但尚未部署或从正式入口验收。53KF 缺少当前账户可执行的只读接口合同和凭据；线索池与销售系统也未形成已验证的数据链路。
- 操作边界：本系统只读监控，调整投放、处理咨询和维护订单仍在来源系统完成。

详细业务口径见[根 README](../README.md)与[项目上下文](../CONTEXT.md)；营销漏斗数据源权威、指标语义和官网无侵入接入红线见[ADR 0001](adr/0001-marketing-funnel-data-source-of-truth.md)；全局 UI 和市场首页的现行设计标准见[视觉设计规范](visual-design-spec.md)；营销第一期的现行需求和技术边界见[营销监控需求目录](blocked-2026-07-29-001-marketing-monitoring/)。跨会话未完成事项统一记录在根目录 [MARK_LATER.md](../MARK_LATER.md)。
...
| 关键词分析 | 摘要、筛选、散点/密度、详情、行动分布和明细表已部署；搜索词独立下钻已完成本地候选实现 | 正式页显示真实关键词；缺少显式行动标签时显示未分类或 `—`，不自动生成建议。搜索词候选只消费同一 Dashboard 的独立 `searchTerms`，不伪造关键词 ID，仍待本次正式部署验收 |
| 网站流量 | 汇总、趋势、来源质量、入口/受访页面、共享筛选和分页已部署 | 正式页返回百度统计访问、UV、PV、质量和页面数据；来源行质量和来源—页面关联没有可信证据时保持缺失 |
| 咨询数据（原始咨询记录） | 表单/在线客服独立口径、趋势/分布、记录表和详情抽屉已部署；九键来源分类与原始 `referrer` 证据展示已进入 `main` | 官网既有代码已部署、本地真实只读验证通过，但生产配置为 `DISABLED`；九键版本尚未部署；53KF 仍为 `NOT_CONNECTED`，页面诚实返回部分覆盖，不渲染 QA fixture |
| 订单结果 | 汇总、来源分布、趋势、筛选、表格、详情抽屉和临时人工关联交互已完成 | 销售系统只读 API 尚未接入；生产态固定显示不可用，完整示例数据只允许在非生产环境显式启用且不会持久化 |

视觉与浏览器证据集中在当前需求目录的 [`design-qa.md`](blocked-2026-07-31-001-market-monitoring-frontend-ia/design-qa.md)。该文件记录的是本地设计验收，不是生产发布凭证；订单结果目前只有源码合同和自动化测试证据，仍待补正式浏览器视觉对比。
...
- `blocked-2026-07-29-001-marketing-monitoring/`：只读市场数据监控定位下的百度第一期；四份搜索推广层级报告及百度统计来源、质量和页面报告已通过严格合同、真实 Token 与正式入口验收，`PILOT_DATA_READY` 仍只向项目白名单开放；Refresh Token 轮换与币种/时区外部确认仍需持续运维
- `blocked-2026-07-31-001-market-monitoring-frontend-ia/`：工作台信息架构与 2026-08-03 按来源完整链路的新版首页视觉/交互已部署，设计稿见[`assets/market-overview-home-final-2026-08-03.png`](blocked-2026-07-31-001-market-monitoring-frontend-ia/assets/market-overview-home-final-2026-08-03.png)。官网代码已部署并与百度 API 分离，但生产只读凭据尚未配置；53KF、线索/订单数据和依赖 KPI 仍待外部条件解除
  - 2026-08-04 真实数据接入逐页盘点见[`data-integration-matrix.md`](blocked-2026-07-31-001-market-monitoring-frontend-ia/data-integration-matrix.md)；Issue 016、017 已关闭，剩余外部阻塞由 Issue 015 跟踪。
- `active-2026-08-04-001-marketing-ai-analysis-report/`：营销数据 AI 分析报告的已确认 PRD、Tech Spec 与实施 issues；已开始仅限非生产环境的前端展示稿实施，后端生成链路尚未实现、接入正式入口或生产验证
