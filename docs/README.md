...
- 当前能力：执行 22 项关键基础检查，返回带版本号的技术健康度；全站报告按规则聚合受影响 URL 并保留逐页账本，单页报告按“检查对象 → 具体问题 → 检测事实 → 建议”展示六类明细和搜索/分享预览
...
- 权重维护：`backend/config/seoAuditRules.js` 集中维护规则版本、严重程度、权重、阈值、页面上限、并发和 Sitemap 深度；Keywords 默认权重为 1
