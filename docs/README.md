...
- 2026-08-05 13:58 CST 已重新核验公开健康与前后端 revision；精确 revision、本地/远端/生产的版本关系及未核验项统一见[部署与运维](DEPLOYMENT.md#当前正式单机实例)。
...
| [Flash 结构化分析可靠性](active-2026-08-05-002-flash-structured-analysis-reliability/prd.md) | 001–008 已完成；009 不批准硬切；011 目标映射歧义隔离已完成并关闭（S55 真实 Flash 3/3 保留目标事实，mapping=ambiguous 不再整条失败）。012 实施 `semantic_evidence_v2`，013 真值审计，014 定向探针，015 独立全量门禁；010 保持阻塞。正式入口仍走 v4，当前 DeepSeek 默认分析配置为 Pro；v5 显式候选固定 Flash。 |
