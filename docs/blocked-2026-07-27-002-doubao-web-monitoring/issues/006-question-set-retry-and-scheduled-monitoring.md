...
把豆包 Web 接入问题集运行、原报告失败重试和项目自动监测，使全部正式入口复用与单问题相同的受管实例、普通模式页面 Adapter、证据语义和错误行为。

运行规划继续沿用现有幂等、配额、任务持久化、暂停、执行租约和终态栅栏。手动运行、完整监测重试和自动监测会在任务创建前实时检查全部相关 Web 平台；任一平台不可用时整体阻断，不创建部分任务或消费配额。任务开始后才发生的失败保留可重试记录。
...
- [x] 任一所选 Web 平台在运行规划前不可用时，整次运行不创建任务、不消费配额，也不跳过该平台继续执行其他平台。
