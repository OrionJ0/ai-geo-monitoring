...
从问题库单问题正式入口打通一条完整的豆包 Web 采集路径：创建空白新会话、主动启用并确认联网搜索、发送问题、等待本次最终回答稳定、提取平台引用、保存联网状态截图与最终截图，并将结果写入现有运行记录。

通用采集状态机负责固定阶段、输入边界、完成判据和 staged evidence 清理；豆包页面实现只负责经过验证的页面交互。DeepSeek Web 应迁移到同一状态机，不能保留第二套旧采集流程。

联网搜索是硬性条件。点击搜索控件本身不代表开启成功；只有读取到唯一、确定的开启状态并保存搜索证据后，系统才能插入和发送问题。问题一旦发送，不得自动再次提交。
...
- [x] 搜索控件缺失、数量不唯一或状态无法确认时任务失败，问题发送次数为 0。
...
阻塞说明：状态机和正式执行器已通过测试，但目标 VM 仍需用真实有引用与无引用回答验证豆包平台引用的最终 DOM 形态；在此之前平台保持关闭。
