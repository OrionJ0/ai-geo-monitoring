...
> 2026-08-05 归属更新：市场工作台父需求中原“正式切流、旧入口退役与生产入口验收”Issue 已在责任移交后删除。百度模块从 `PILOT_DATA_READY` 提升到零阻塞 `VERIFIED`、关闭试点模式并进入 `READY` 的全部剩余责任，由本 issue 与前置 Issue 008 统一管理；生产已有真实数据不等于模块已无条件正式就绪。
