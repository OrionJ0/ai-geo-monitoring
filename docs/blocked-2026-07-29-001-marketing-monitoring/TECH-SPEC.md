...
- 状态：Blocked（白名单百度真实数据已部署并通过正式入口验收；正式 `READY` 契约门禁与剩余外部来源仍阻塞）
...
## 19. 2026-07-30 白名单真实数据试点（历史 / 已退役凭据模型）
...
- 历史试点当时使用百度营销 dev2 OAuth Token 与百度统计商业账号 Data API Token 两套凭据。该凭据模型已于 2026-08-05 硬切退役；当前生产只保存一套统一 OAuth Access/Refresh Token，百度统计另保存非秘密用户名并以当前 Access Context 校验。
...
- GoodieAI 后端只调用官网聚合统计接口，使用部署环境注入的服务凭据、服务端 JWT 缓存、响应结构校验、日期范围来源聚合快照和最后成功数据；常规同步不得读取联系人明细，前端不得直接登录官网后台。最多 31 日的逐日表单趋势已完成真实区间对账且代码已部署；生产仍需注入专用项目与最小权限只读身份后才能把模块从 `DISABLED` 切为 `READY`。
