...
type: HITL
blocked_by:
  - "009-flash-41x3-comparison-gate.md"
---
...
仅在真实 Flash 对比全部硬门槛通过后，把 v5 设为所有新分析的唯一正式路径。单问题、问题集、自动监测和 analysis-only 必须统一写入 v5，并通过请求审计证明实际使用固定 Flash 两阶段合同。

切换完成后退役 v4 运行时及其专属提示、修复分支、默认值、隐藏开关、fallback、测试和现役文档；只保留读取历史 v4 数据所需的明确版本化校验器。发布使用项目正式流程，生产问题默认修复 v5，不恢复静默旧路径。

## Acceptance criteria

- [ ] 009 的全部硬门槛均为 PASS，并有明确的人工作出上线批准；否则本 issue 不得开始实施。
...
- [009-flash-41x3-comparison-gate.md](009-flash-41x3-comparison-gate.md)
