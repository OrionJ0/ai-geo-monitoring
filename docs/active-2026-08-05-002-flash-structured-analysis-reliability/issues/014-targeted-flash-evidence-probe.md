...
status: completed
type: AFK
note: "2026-08-06 完成：结构探针通过（确定性轨道全绿）；rev2 为提示词最后一轮定向回归，全部验收通过——冻结 v5-json-rev2 作为 015 语义提示词版本；不再有 rev3/rev4，不引入多数表决"
blocked_by:
...
## 第 3 轮（最后一轮）rev2 定向 A/B（运行前冻结 2026-08-06）

数据所有者裁决：选 A（修订提示词）但为**最后一轮、严格限界 rev2**；不同意多数表决——三次投票反而会得出相反结论，且生产只调用一次，评测用投票会掩盖真实单次风险。rev1 的问题是提示词串线（"综合性较强≠推荐"在推荐字段正确，但模型把"不推荐"错误传递成"情绪中性"），正确组合应为 `recommendation=false, sentiment=positive`，这是可修复的明确串线，值得最后一轮验证；但它不继续阻塞项目——rev2 后必须进入 015，让全量评测决定是否硬切。

- **修订范围（仅情绪规则 + 示例替换）**：
  1. 明确加入：推荐与情绪是两个相互独立的判断维度；未推荐、未表达购买偏好，不等于中性评价。
  2. `"综合性较强""能力突出""覆盖完整""稳定成熟"`等肯定能力或优势的描述应判为 positive，即使 recommendation=false。
  3. 只有纯粹陈述存在、功能、规格或名单且没有价值判断时，才判为 neutral。
  4. 增加组合示例：`{"text": "Goodie AI 综合性较强，但应根据具体需求选择", "recommendation": false, "sentiment": "positive"}`。
  5. 删除推荐规则里孤立的"综合性较强≠推荐"表述（rev1 串线根因），推荐规则反例改为"两者各有优劣""你的选择应取决于具体需求"。
- 样本：S07、S12、S43、S55；臂：`v5-json` + `v5-json-rev2`；每条 3 次；共 **24 次真实 `deepseek-v4-flash` 调用**（0 缓存，输出 `work/geo-flash-probe-014-2026-08-06/ab-rev2/`）。
- 验收重点：S12 `false + positive`；S07 显式推荐仍为 `true + positive`；S43/S55 不得回归；target_fact 继续保持 100%；`evidence_reference_invalid=0`；不得新增整条失败。S12 达到 3/3 是目标但只是定向回归，不构成"证明达到 99%"。
- **决策规则（数据所有者裁决，不再有 rev3/rev4）**：rev2 通过或仅 1 次波动 → 冻结 rev2（波动记录为 Flash 单次语义方差，用全量数据衡量）进入 015；明显退化 → 回退基线提示词进入 015。当前正式入口仍 v4；010 继续阻塞。

## 第 3 轮（最后一轮）rev2 A/B 结果（2026-08-06，24 次真实 deepseek-v4-flash 调用）

运行目录：`work/geo-flash-probe-014-2026-08-06/ab-rev2/`（两臂各 12 次，`from_cache` 全 0，全部真实调用）。

| 检查项 | v5-json（对照） | v5-json-rev2（修订） |
| --- | ---: | ---: |
| 整条完成率 | 100%（12/12） | 100%（12/12） |
| target_fact 与确定性扫描一致（确定性稳定性） | 12/12 对 = 100% | 12/12 对 = 100% |
| `analysis_evidence_reference_invalid` | 0 | 0 |
| 降级 | 0 | 0 |
| S12 推荐（真值 false） | 3/3 false ✓ | **3/3 false ✓** |
| S12 情绪（真值 positive） | 3/3 positive ✓ | **3/3 positive ✓** |
| S07 推荐/情绪（真值 true/positive） | 3/3 ✓ | 3/3 ✓ |
| S43 推荐/情绪（真值 true/positive） | 3/3 ✓ | 3/3 ✓ |
| S55 | 3/3 target_fact 正常、语义轨诚实 unavailable（mapping 歧义，不回归） | 同左 |
| Token 中位 / 延迟中位 | 10090 / 8708ms | 5510 / 6629ms |

**验收：全部通过。** S12 `false + positive` 达到 3/3（推荐与情绪两字段与真值一致）；S07 显式推荐仍为 true + positive；S43/S55 无回归；target_fact 100%；evidence 引用错误 0；无新增整条失败。

**rev1→rev2 串线修复验证**：rev1 批次中 rev1 臂 S12 情绪 1/3（"'综合性较强'=中性评价，未表达明确推荐或偏好"——推荐示例串线），rev2 批次中 rev2 臂 3/3 且理由恢复正常（positive 依据为"综合性较强、两环节都有覆盖"的肯定性描述），rev1 的推荐→情绪串线已消除。

**决策（数据所有者规则，最后一轮）：rev2 通过 → 冻结 `v5-json-rev2` 作为 015 语义提示词版本，进入 015。** 两点诚实限定：① S12 达到 3/3 只是定向回归结果，不构成"证明达到 99%"，重复一致率由 015 全量 41×3 门禁衡量；② 对照臂本批 S12 情绪也 3/3（rev1 批次该臂 2/3），证明情绪波动含有模型随机性成分——rev2 的价值是消除 rev1 的确定性串线退化（同批内 rev1 曾 1/3 < 对照 2/3），而非宣称让随机波动归零。不再有 rev3/rev4，不引入多数表决。当前正式入口仍走 v4；010 继续阻塞。
