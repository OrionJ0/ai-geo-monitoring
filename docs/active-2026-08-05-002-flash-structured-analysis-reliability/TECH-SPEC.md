...
### 7.7.1 2026-08-06 issue 015 全量门禁结果（真值冻结后，324 次真实调用）

数据所有者（OrionJ0）授权冻结 truth v3（55 条 confirmed，preflight PASS）后，对 54 条去重样本（55 − S19/S20 + C01）× 3 × 2 臂运行 v5-json-rev2 vs v4-current（0 缓存）。

- **硬门槛**：完成率 162/162 PASS；target_fact 100%/FP=0 PASS；evidence/grounding 0 PASS；已输出关系 precision **92.39% < 95% FAIL**（TP=995，FP=82，FN=478）；Token PASS（中位 5510 ≤ 11303，p95 8960 ≤ 37167）。
- **语义门槛**：推荐 F1 **83.69% < 95% FAIL**（TP=59，FP=17，FN=6；FP 集中在 S46/S29/S54/S16/S27/S49 的"场景适配/列举"误推荐）；情绪 accuracy **100% PASS**（92/92，32 唯一样本）；排名 **NOT EVALUABLE（0/6，真值仅 6 条）——排名能力证据不足，非阻塞观察指标**；target_mapping 0/3（S53 预测 resolved vs truth conflicting_identity，真值仅 1 条）。
- **诚实降级**：推荐/情绪/排名各 4/4/3 条，单独计数、不计错误、不伪装 assessed。
- **重复运行**：逐次计分方差（推荐 F1 mean 0.837±0.013；情绪 mean 0.958±0.018），禁止多数投票。

**决策：不批准硬切，010 保持阻塞。** v5-json-rev2 结构轨道全绿，真实缺口定位在推荐口径过宽与竞品关系判定；按数据所有者裁决不重开提示词（无 rev3/rev4）、不引入多数投票。正式入口仍 v4。

### 7.7.2 2026-08-06 发布合同修订与确定性问题修复（数据所有者裁决）

数据所有者裁决：015 按冻结门槛（推荐/关系 95%）确实 FAIL、历史不改写；但 v5 实际明显优于 v4（完成率 100% vs 87.65%、核心稳定率 97.53% vs 67.18%、Token 5510 vs 7535、延迟 7.0s vs 10.2s、v4 20 条结构化失败 vs v5 0），继续长期使用 v4 不是更好的产品选择。发布合同改为产品目标导向：**v5 上线硬门槛** = 完成率、目标事实、grounding、证据、成本（015 实测全部 PASS）；**AI 最佳努力指标** = 推荐 F1 83.69%、竞品关系 precision 92.39%、情绪 accuracy 100%（公布实测水平，不宣称 95%）；竞品遗漏不导致整条失败；排名发布口径"证据不足"。

两个确定性问题已修复（非 rev3/rev4，不修改语义提示词、不引入多数投票），21 次定向回归通过：

- **S53 法律主体冲突**：`buildEntityCatalog` 识别完整法律主体中的同名短词（"深圳市广拓科技有限公司"≠目标"上海广拓"）→ `target_mapping=conflicting_identity`、语义轨 unavailable；S53 3/3 与 truth 对齐。
- **排名 0/6 链路**：中文数字名次（"排名第一"）、目标 mention 行扫描（含空格/markdown 连接容忍）、梯队/排序声明无法提取时 unavailable；S49/S50/S02 rank=1 正确提取、S01/S46 诚实 unavailable。

**7.6 门槛修订生效范围**：预注册上线门槛中推荐 F1≥0.95 与关系 precision≥0.95 项对 v5 上线不再作为硬性 PASS/FAIL，转为最佳努力报告；其余门槛（完成率/target_fact/grounding/证据/Token/注册表/降级/稳定性）保持。010 已按此合同 in_progress。
