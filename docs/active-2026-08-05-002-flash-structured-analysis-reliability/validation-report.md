...
v5 阶段 2 在 59.3%（73/123）样本上因 `analysis_evidence_reference_invalid` 降级：严格证据校验（issue 002 删除自动补证据）要求证据 source_id 必须包含实体出现片段，而真实 Flash 输出的证据经常不满足，repair 后仍失败即降级。降级使推荐/排名/情绪字段为 `unresolved`，目标核心签名因降级随机性而不稳定。

**结论：不批准硬切。** 生产继续使用 `ai_structured_v4` / `deepseek-v4-flash`（v4 分析器）；v5 候选需要改进阶段 2 提示词以生成可锚定证据，或调整证据校验合同后按新修订重跑。010 保持阻塞。
