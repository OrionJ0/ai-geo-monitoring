...
This project's current product identity is a read-only market data monitoring system, not a single-purpose AI GEO/SEO monitor. It uses a Next.js 16.2.12 frontend (App Router) and Express.js backend. GEO/SEO is a verified formal workflow, but that delivery stage does not redefine the product boundary. The complete market workspace pages are deployed. Production live verification covers the four-report Baidu advertising snapshot, Baidu Tongji traffic, and existing AI/GEO data. The separate website-form aggregate and masked-record contracts are deployed and locally live-verified, but production keeps them `DISABLED` until a dedicated read-only website project and account credentials are injected. 53KF remains independently `NOT_CONNECTED`; lead-pool and order facts remain unavailable and render honest missing states. The order fixture is development-only. Website form consultations never include 53KF chats or all historical form records. `../docs/README.md#当前前端页面实施状态`, `../docs/visual-design-spec.md`, the root `README.md`, and `CONTEXT.md` are the current authorities. Legacy repository, service, page, or heading names containing `AI GEO` do not override this positioning.
