...
test('问题集报告分级展示指标并给出简短的悬停口径说明', () => {
  ...
  assert.match(source, /明确推荐数 ÷ 推荐语义已评估数；未评估不入分母/);
...
test('原生问题集报告可以确认后重试失败项', () => {
  ...
  assert.match(source, /当前设置中心的模型和参数/);
