...
test('workspace sidebar stays operable and named on narrow screens', () => {
  ...
  assert.match(globalStyles, /\.workspace-navigation > \.ant-menu-item-group > \.ant-menu-item-group-title\s*\{[^}]*color:\s*#8993a4[^}]*font-size:\s*14px/);
  assert.match(globalStyles, /\.workspace-navigation\s*\{[\s\S]*background:\s*#f3f5f8/);
  ...
  assert.match(globalStyles, /\.workspace-navigation \.ant-menu-item-group-list \.ant-menu-item\s*\{[\s\S]*padding-inline-start:\s*36px/);
