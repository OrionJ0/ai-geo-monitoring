...
- 渠道关系：全链路数据的渠道目录内置为百度推广、直接访问、百度搜索、必应搜索、Google 搜索、其他搜索和外部引荐。百度推广的投入/展现来自百度营销，站内访问来自百度统计；其他渠道的访问也只来自百度统计。直接访问不标为自然搜索。只有存在可信来源键或经确认的人工映射时，下游记录才进入对应渠道链路；不得把百度统计与官网流量相加、平均或补差，也不得把同期广告、全站访问、咨询和订单直接拼接成归因事实。
