...
> **现行产品定位（2026-08-03）**：本项目已从单一 GEO/SEO 监测更新为面向市场负责人的**只读市场数据监控系统**。GEO/SEO 是当前已经验证的一组正式能力，不是整个项目的唯一边界；仓库中的市场工作台与新版首页已完成视觉和交互实现，但不得据此推断数据全链路或生产部署已经完成。

GoodieAI 在同一个项目视角下逐步联合观察 GEO/SEO，以及按来源归属的展现、访问（点击）、官网表单咨询、在线客服咨询、线索入池、成交订单数和成交订单金额。系统只读取、保存和分析来源事实；投放调整、咨询处理、线索与机会推进、订单维护仍在各来源系统完成。
...
| 最终全链路目标 | 按来源联合观察展现、访问（点击）、客服咨询、线索入池、成交订单数与成交订单金额 |
...
- 来源关系：只有存在可信来源键或经确认的人工映射时，记录才进入对应来源链路。百度统计 `source/all/a`、`source/engine/a` 已真实验证的来源标识可拆为百度搜索、必应搜索、直接访问和其他来源；其他来源包含非百度/必应搜索与外部链接，百度付费推广必须排除。不得把百度统计与官网流量相加、平均或补差，也不得把同期广告、全站访问、咨询和订单直接拼接成归因事实。

当前状态必须区分：

- GEO/SEO 是现有正式工作流。
- 百度营销的 OAuth、搜索账户绑定、最近 30 日搜索推广快照和百度统计读取已进入白名单真实数据试点。本地广告代码已硬切为推广计划、推广单元、关键词、搜索词四份官方报告的同次原子刷新，广告表现可严格下钻到关键词；搜索词因官方响应没有关键词 ID 而保持独立事实。2026-08-03 已用服务器现有生产 Token 完成四份报告的只读真实响应验证，但第二阶段业务变更尚未推送或部署，生产仍是旧版本，正式开放状态须从正式入口重新验证。
- 2026-08-04 已从官网生产入口只读验证聚合统计、联系人列表和按需详情接口，并在本地工作区实现独立官网聚合适配器、最多 31 日逐日聚合、版本化缓存，以及 `/api/consultations` 的脱敏记录级 adapter；真实区间聚合、3 条记录、详情和本地 production build 页面已完成对账，本地页面所选 30 日原始表单列表另返回 20 条。首页常规统计不读取联系人明细，记录页只按需读取并在服务端脱敏；两条链路均不与 `/api/marketing` 或 53KF 混用。第二阶段业务变更尚未推送、部署、配置生产凭据或完成正式入口验收。
- 2026-08-03 已确认并实现新版首页设计和指标口径，详见[全局视觉设计规范](docs/visual-design-spec.md)。投放效率、来源全链路、每日趋势、面包屑、响应式与无障碍已通过本地测试；官网表单列只覆盖可归因成功提交会话，不包含 53KF。线索/订单真实数据和依赖这些字段的指标仍未接入，生产运行需独立验证。
- 2026-08-04 已复核本地市场工作台页面完成度：市场总览、广告表现、关键词分析、网站流量、原始咨询、订单结果及共享导航外壳均已有设计实现，但数据成熟度不同。原始咨询已有官网聚合及脱敏记录级真实数据，53KF 仍为独立 `NOT_CONNECTED`；订单结果仍无生产数据源。完整逐页状态以[文档总览中的前端页面实施状态](docs/README.md#当前前端页面实施状态)为准。
...
- 百度营销真实数据试点：显式迁移、OAuth/Token、搜索账户绑定、30 日计划/单元/关键词/搜索词原子快照、精确值看板和刷新生命周期均已在本地实现并完成真实响应只读验证；严格层级只到关键词，搜索词不伪造关键词 ID。`PILOT_DATA_READY` 使用百度营销 dev2 OAuth Token 读取搜索推广，另用百度统计“数据 API”签发的商业账号 Token 展示来源、PV、UV、访问次数、全站质量和页面报告，两套凭据分别加密保存且不得混用。仍待币种/时区、Refresh Token 轮换、部署和正式入口完整验收
- 官网表单咨询本地接入：独立 `websiteFormConsultations` 模块只读调用官网现有聚合接口，公开区间汇总 `/api/website-data/projects/:projectId/form-consultations` 和最多 31 日逐日接口 `/api/website-data/projects/:projectId/form-consultation-days`；只返回来源级可归因成功提交会话，不返回联系人明细、官网流量或 53KF 数据。官网模块使用独立配置、状态、缓存仓储和迁移账本，不进入百度 `/api/marketing` 模块
- 官网原始咨询本地接入：独立 `consultationRecords` 模块只读调用官网联系人列表和按需详情接口，通过 `/api/consultations/projects/:projectId/records` 输出严格、服务端脱敏的浏览合同；任何上游调用前校验项目所有权，详情审计失败时拒绝返回。该链路不读取 53KF，也不把联系入口点击、自动问候或窗口打开计为咨询
...
- 按百度推广、直接访问、搜索引擎、外部链接等当前可证明粒度查看链路；只有可信来源关系成立时才进入对应来源行，引擎级自然来源需等待可信来源键
