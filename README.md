...
- 平台级模型请求参数配置、连接测试与联网能力检测；千问 Responses 新预置默认强制搜索；腾讯混元新预置使用 TokenHub `hy3-preview` 并默认携带 `web_search_options.enable=true`；DeepSeek API 新预置和统一发布候选结构化分析固定使用 `deepseek-v4-flash`，未配置密钥时保持关闭；生产启动桥 `387ae45` 仍运行 v4/Pro
- 管理员可在设置中心查看两个网页版平台的浏览器、Profile 和登录验证状态，打开各自专用 Chrome 人工登录或切换账号
- 统一发布候选由唯一 `ai_structured_v5` 两阶段分析器读取当前问题与完整原回答：阶段 1 抽取可落到原文的实体，阶段 2 在封闭实体目录内判断竞争、推荐和情绪；程序计算目标事实与开放发现范围内的 SOV。候选分析身份固定为官方内置 DeepSeek `deepseek-v4-flash`，进程级共享并发和有界队列覆盖所有入口，不存在 v4/Pro fallback；生产启动桥 `387ae45` 仍运行 v4/Pro，须待统一候选发布和四入口验收后才成为正式路径
...
统一发布候选中的新运行固定使用 `ai_structured_v5`、`geo_metric_input_v5`、`three_track_partial_v2` 和 `contextual_competitor_mentions_sov_v2_scoped`，并在创建记录时冻结竞品注册表快照；analysis-only 只复用原回答、引用和冻结快照，不重新调用监测上游或消耗监测配额。完整原回答不会在应用层静默截断，阶段失败按字段诚实降级，不会恢复 v4、Pro 或旧 SOV。生产启动桥 `387ae45` 在统一候选发布前仍运行 v4/Pro；历史 v4/v1 及更早结果保持原值，只通过报告和导出兼容读取。
