...
- 百度 Provider 当前保持一个兼容 `BaiduMarketingClient` facade，由它委托独立的 OAuth、搜索推广和百度统计客户端；三个产品客户端共用唯一 `BaiduHttpKernel` 安全内核。公开 API、数据库、页面和指标合同不变，旧单体产品逻辑、重复安全实现与 runtime fallback 已退役。
...
- 2026-08-06 已通过正式 Git Bundle 发布百度 Provider 模块化 revision `2c6a36e4018d36d926a44a1ad2fc8825b7320635`，并重新核验服务器工作区、迁移、systemd、统一 OAuth、四报表、百度统计、原子快照、真实 Chrome 页面与 Network。精确证据统一见[部署与运维](docs/DEPLOYMENT.md#当前正式单机实例)。
...
- 平台级模型请求参数配置、连接测试与联网能力检测；千问 Responses 新预置默认强制搜索；腾讯混元新预置使用 TokenHub `hy3-preview` 并默认携带 `web_search_options.enable=true`；DeepSeek API 新预置和正式结构化分析固定使用 `deepseek-v4-flash`，未配置密钥时保持关闭
- 管理员可在设置中心查看两个网页版平台的浏览器、Profile 和登录验证状态，打开各自专用 Chrome 人工登录或切换账号
- 正式结构化分析由唯一 `ai_structured_v5` 两阶段分析器读取当前问题与完整原回答：阶段 1 抽取可落到原文的实体，阶段 2 在封闭实体目录内判断竞争、推荐和情绪；程序计算目标事实与开放发现范围内的 SOV。分析身份固定为官方内置 DeepSeek `deepseek-v4-flash`，进程级共享并发和有界队列覆盖所有入口，不存在 v4/Pro fallback；2026-08-07 统一 v5 已正式发布并经四入口线上验收，v4/Pro 运行时已退役
...
竞品由分析模型依据当前问题与回答语境逐实体判断；项目中人工配置的竞品不会进入模型提示，只在两个模型阶段之间由程序执行身份归一，并继续用于引用来源归类。目标品牌与竞品都未提及时，该回答的 SOV 为 `N/A`；项目或问题集的平均 SOV 是所有可计算单回答 SOV 的等权平均，不把全部回答的提及次数合并后再除。

正式运行的 v5 新记录固定使用 `ai_structured_v5`、`geo_metric_input_v5`、`three_track_partial_v2` 和 `contextual_competitor_mentions_sov_v2_scoped`，并在创建记录时冻结竞品注册表快照；analysis-only 只复用原回答、引用和冻结快照，不重新调用监测上游或消耗监测配额。完整原回答不会在应用层静默截断，阶段失败按字段诚实降级，不会恢复 v4、Pro 或旧 SOV。历史 v4/v1 及更早结果保持原值，只通过报告和导出兼容读取。
