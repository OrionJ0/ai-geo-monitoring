...
GoodieAI 在同一个项目视角下逐步联合观察 GEO/SEO，以及按来源归属的展现、访问（点击）、官网客服咨询、线索入池、成交订单数和成交订单金额。系统只读取、保存和分析来源事实；投放调整、咨询处理、线索与机会推进、订单维护仍在各来源系统完成。
...
- 百度统计与官网：读取自然搜索来源的访问事实，以及官网客服咨询；付费广告的“访问（点击）”与广告点击采用同一阶段，避免重复计数。
- 客服与销售线索池：客服咨询由人工确认并转换为销售系统中的线索；“线索入池数”是必须监控的链路阶段。
- 内部销售系统：未来 API 开放后读取最小稳定订单标识、成交订单数和成交订单金额；订单数用于 CPA、成交率和整体转化率，订单金额用于 ROAS。
- 来源关系：只有存在可信来源键或经确认的人工映射时，记录才进入对应来源链路；不得把同期广告、全站访问、咨询和订单直接拼接成归因事实，也不做未经确认的自动身份匹配、多触点归因或来源系统回写。

当前状态必须区分：

- GEO/SEO 是现有正式工作流。
- 百度营销的 OAuth、搜索账户绑定、最近 30 日搜索推广快照和百度统计读取已进入白名单真实数据试点；本地代码已经展示完整工作台导航，但生产开放状态仍须从正式入口重新验证。
- 客服咨询、线索入池、销售系统订单数、订单金额和跨系统来源关系尚未形成已验证的完整数据合同；当前页面不得把试点数据描述成已经打通的完整营销漏斗。
- 2026-08-03 已确认并实现新版首页设计和指标口径，详见[全局视觉设计规范](docs/visual-design-spec.md)。投放效率、来源全链路、每日趋势、面包屑、响应式与无障碍已通过单元测试、生产构建和真实 Chrome/axe 验收；客服/线索/订单真实数据和依赖这些字段的指标仍未接入，生产运行需独立验证。
...
- 百度营销真实数据试点：显式迁移、OAuth/Token、搜索账户绑定、30 日推广计划原子快照、精确值看板和刷新生命周期均已实现；`PILOT_DATA_READY` 使用百度营销 dev2 OAuth Token 读取搜索推广，另用百度统计“数据 API”页面签发的商业账号 Token 实时只读展示 PV、UV 与访问次数，两套凭据分别加密保存且不得混用。工作台已展示完整页面入口，币种/时区、Refresh Token 轮换和完整生产验收仍是百度真实数据进入 `READY` 的门禁
...
- 按百度推广、百度自然搜索、必应自然搜索等来源查看完整链路；只有可信来源关系成立时才进入对应来源行
