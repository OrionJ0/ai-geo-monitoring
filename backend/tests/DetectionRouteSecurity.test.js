...
test('detection json errors do not expose internal exception messages', () => {
  assert.doesNotMatch(routeSource, /error:\s*error\.message/);
});

test('detection records store safe platform failure messages', () => {
...
test('detection stream errors do not expose provider exception messages', () => {
...
test('detection validates explicit dynamic platforms instead of silently dropping them', () => {
