...
test('website traffic APIs authorize the project and retired Tongji routes stay unavailable', async (t) => {
    ...
    tongjiService: {
      async readProjectWebsiteTraffic(projectId, options) {
        ...
        if (
          ...
          && options.source === 'ALL'
          && options.includeSourceComparison === 'true'
        ) {
  ...
  const invalidPartitionResponse = await fetch(
    `http://127.0.0.1:${address.port}/api/marketing/projects/11/website-traffic-overview?device=mobile&from=2026-07-01&to=2026-07-30&source=ALL&metric=visits&includeSourceComparison=true`
  );
