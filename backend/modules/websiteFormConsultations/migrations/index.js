...
function loadWebsiteDataMigrations({ directory = __dirname } = {}) {
  return fs.readdirSync(directory)
    ...
    .map((filename) => {
      ...
      if (typeof migration?.up !== 'function') {
        const error = new Error(`官网数据迁移缺少 up(): ${filename}`);
        error.code = 'WEBSITE_DATA_MIGRATION_INVALID';
        throw error;
      }
...
module.exports = { loadWebsiteDataMigrations };
