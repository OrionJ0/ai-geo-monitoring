...
const { loadConsultationRecordMigrations } = require('./index');

const LEDGER_TABLE = 'consultation_record_schema_migrations';
...
function createConsultationRecordMigrationRunner({
  sequelize,
  migrations = loadConsultationRecordMigrations()
} = {}) {
  if (!sequelize) {
    throw migrationError(
      '咨询记录迁移缺少数据库连接',
      'CONSULTATION_RECORD_DATABASE_REQUIRED'
    );
  }
  ...
  for (const migration of orderedMigrations) {
    if (
      ...
      throw migrationError(
        '咨询记录迁移定义无效',
        'CONSULTATION_RECORD_MIGRATION_INVALID'
      );
    }
  ...
  function validateRows(rows) {
    ...
    for (const row of rows) {
      ...
      if (!migration) {
        throw migrationError(
          `数据库包含未知的咨询记录迁移: ${row.version}`,
          'CONSULTATION_RECORD_MIGRATION_UNKNOWN'
        );
      }
      if (row.checksum !== migration.checksum) {
        throw migrationError(
          `咨询记录迁移 checksum 不一致: ${row.version}`,
          'CONSULTATION_RECORD_MIGRATION_CHECKSUM_MISMATCH'
        );
      }
    }
  }

  function snapshot(rows, ledgerPresent = true) {
    const appliedVersions = rows.map((row) => row.version);
    const applied = new Set(appliedVersions);
    return {
      ready: ledgerPresent && appliedVersions.length === orderedMigrations.length,
      ledgerPresent,
      appliedVersions,
  ...
  async function audit() {
    ...
    if (!['version', 'checksum', 'applied_at'].every((key) => description[key])) {
      throw migrationError(
        '咨询记录迁移账本结构不完整',
        'CONSULTATION_RECORD_MIGRATION_LEDGER_INVALID'
      );
    }
  ...
  async function apply() {
    ...
    if (!['sqlite', 'postgres'].includes(dialect)) {
      throw migrationError(
        '咨询记录迁移不支持当前数据库方言',
        'CONSULTATION_RECORD_DATABASE_DIALECT_UNSUPPORTED'
      );
    }
    if (dialect === 'sqlite') {
      ...
      await sequelize.transaction(async (transaction) => {
        await sequelize.query(
          "SELECT pg_advisory_xact_lock(hashtext('ai_geo_consultation_record_schema_migrations'))",
          { transaction }
        );
